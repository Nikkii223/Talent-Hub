...
public class VideoPlayerRecyclerView extends RecyclerView {
    ...
    private enum PlayerState {ON, OFF};
    ...
    // vars
    private ArrayList<Content> mediaObjects = new ArrayList<>();
    ...
    private void init(Context context){
        ...
        setVolumeControl(PlayerState.ON);
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    ...
                    if(!recyclerView.canScrollVertically(1)){
                        ...
                        playVideo(true);
                        //setVolumeControl(PlayerState.OFF);

                    }
                    else{
                        //playerState = PlayerState.ON;
                        playVideo(false);
                        //setVolumeControl(PlayerState.ON);
    ...
    public void playVideo(boolean isEndOfList) {
      ...
      thumbnail = holder.thumbnail;
        ...
        playerControl = holder.playerControl;
        ...
        if(viewHolderParent!=null ){
            ...
            //playerControl.setOnClickListener(videoViewClickListener);
    ...
    private void togglePlayerStatus(){
        if (videoPlayer != null) {
            if (playerState == PlayerState.OFF) {
                Log.d(TAG, "togglePlaybackState: enabling volume.");
                setVolumeControl(PlayerState.ON);

            } else if(playerState == PlayerState.ON) {
   ...
   /* private void toggleVolume() {
    ...
    private void setVolumeControl(PlayerState state){
        playerState = state;
        if(state == PlayerState.OFF){
            //videoPlayer.setVolume(0f);
            //animateVolumeControl();
            videoPlayer.setPlayWhenReady(false);
            animatePlayerControl();
        }
        else if(state == PlayerState.ON){
    ...
    private void animatePlayerControl(){
        if(playerControl != null){
            ...
            playerControl.animate().cancel();

            playerControl.setAlpha(1.0f);

            playerControl.animate()
                    .alpha(0f)
    ...
    public void updateVideoPlayerStatus(boolean status){
        if(videoPlayer!=null){
            videoPlayer.setPlayWhenReady(status);
            if(!status)
                videoSurfaceView.getVideoSurfaceView().setVisibility(GONE);
            else
                videoSurfaceView.getVideoSurfaceView().setVisibility(VISIBLE);
        }
